...
def check_brackets(list_expr):
    ...
    for offset, num in enumerate(list_expr):
        if num == '(':
            ...
            rightbr.append(num)
        elif ((num in DICT_MATH or          # just 2 + abs
              num in DICT_ABS or            # (if some math expression
              num in DICT_ROUND) and offset == len(list_expr)-1):
            raise check_bracketsException   # stands in the end)
        elif ((num in DICT_MATH or          # not 'abs('
              num in DICT_ABS or            # (if '(' doesn't stand
              num in DICT_ROUND) and list_expr[offset+1] != '('):
            raise check_bracketsException   # after some math expression)
    if len(leftbr) != len(rightbr):
        raise check_bracketsException


def check_mathseparatrix(list_expr):            # checks ','
    for offset, num in enumerate(list_expr):    # in args DICT_MATH_ARGS
        if num in DICT_MATH or num in DICT_MATH_ARGS:
            check_list = list_expr[offset+1:]   # and DICT_MATH
            ...
            for sign in check_list:
                if sign == ',':
                    countsep += 1
                if sign in DICT_MATH_ARGS:
                    countdict += 1

            if num in DICT_MATH and countsep > countdict+1:     # countdict+1
...
def check_comparison(list_expr):
    ...
    for num in list_expr:
        if num in BOOL_DICT:
            count += 1
    if count > 1:
        raise check_comparisonException
