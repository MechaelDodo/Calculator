...
class Convert:
    ...
    def convert_digit(self):        #returns new list(doesn't change list): ['3','3'] => ['33']
        index = 0
        digitlist = []
        list_expr_res = []
        while True:
            if index == len(self.list_expression):
                ...
                digitlist = []
                break
            if not str(self.list_expression[index]).isdigit() and bool(digitlist):           
                digitlist = ''.join(digitlist)
                list_expr_res.append(digitlist)
                list_expr_res.append(self.list_expression[index])
                index += 1
                digitlist = []
            elif not str(self.list_expression[index]).isdigit() and not digitlist:
                list_expr_res.append(self.list_expression[index])
                index += 1
            else:
